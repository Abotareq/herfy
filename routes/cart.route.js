...
/**
 * @route POST /cart
 * @desc Create a new cart if not exists, don't use it to add items
 */
router.post("/", validate(createOrUpdateCartSchema), cartController.createOrUpdateCart);

/**
 * @route PATCH /cart
 * @desc Update general cart data (e.g. coupon), not items
 */
router.patch("/", validate(updateCartSchema), cartController.updateCart);
...
/**
 * @route DELETE /cart
 * @desc Delete entire cart only after checkout
 */
router.delete("/", cartController.deleteCart);

/**
 * @route POST /cart/items
 * @desc Add an item to cart or update quantity if exists
 */
router.post("/items", validate(addItemSchema), cartController.addItemToCart);

/**
 * @route DELETE /cart/items/:productId
 * @desc Remove an item from cart by productId
 */
